...
func parseFeed(feedurl string) parsedFeed {
	...
	for _, item := range feed.Items {
		itemDate, err := time.Parse(time.RFC3339, item.Extensions["dc"]["date"][0].Value)
		if err != nil {
			panic(err)
		}
		if compareTime.Before(itemDate) {
...
func formatOutput(feeds []parsedFeed) string {
	...
	for _, feed := range feeds {
		if len(feed.items) > 0 {
			if result == "" {
				result = fmt.Sprintf("# Pinboard articles for %s", time.Now().Format(settings.Dateformat))
			}
